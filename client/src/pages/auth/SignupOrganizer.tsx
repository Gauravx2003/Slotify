...
import { Mail, Lock, User, Camera, Eye, EyeOff, ArrowRight, TrendingUp, BarChart3, Users } from 'lucide-react';
...
export default function SignupOrganizer() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Dispatch with role 'organiser'
        const result = await dispatch(signup({ ...signupData, role: 'organiser' }));
        ...
        if (signup.fulfilled.match(result)) {
            ...
            navigate('/verify-otp', { state: { email: formData.email, role: 'organiser' } });
        }
    };
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 bg-white/70 backdrop-blur-xl border border-white/60 rounded-[2.5rem] shadow-2xl overflow-hidden min-h-[600px] max-w-6xl mx-auto w-full">
            {/* Left Side - Visuals (Organizer specific) */}
            <div className="hidden md:flex flex-col justify-between p-12 bg-rust-50 relative overflow-hidden">
                {/* Background Blobs */}
                <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
                    <div className="absolute top-[-20%] right-[-20%] w-[80%] h-[80%] bg-rust-200/20 rounded-full blur-[100px]" />
                    <div className="absolute bottom-[-20%] left-[-20%] w-[80%] h-[80%] bg-accent-yellow/10 rounded-full blur-[100px]" />
                </div>
                ...
                <div className="relative z-10">
                    ...
                    <h2 className="text-4xl font-black text-surface-900 mb-6 leading-tight">
                        Grow Your <br /><span className="text-rust-600">Business</span>
                    </h2>
                    <p className="text-surface-600 text-lg leading-relaxed max-w-md">
                        Manage appointments, resources, and clients all in one powerful platform. Built for professionals.
                    </p>
                </div>

                <div className="relative z-10 mt-12 grid grid-cols-2 gap-4">
                    <div className="bg-white/60 p-4 rounded-2xl border border-white/60">
                        <BarChart3 className="w-6 h-6 text-rust-500 mb-2" />
                        <p className="font-bold text-surface-900">Analytics</p>
                        <p className="text-sm text-surface-500">Insights & Reports</p>
                    </div>
                    <div className="bg-white/60 p-4 rounded-2xl border border-white/60">
                        <Users className="w-6 h-6 text-rust-500 mb-2" />
                        <p className="font-bold text-surface-900">Clients</p>
                        <p className="text-sm text-surface-500">Management</p>
                    </div>
                </div>
                    ...
                    <div className="relative rounded-2xl overflow-hidden shadow-xl border-4 border-white/50">
                        ...
                        <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-md p-3 rounded-xl shadow-lg border border-white/50 flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-rust-100 flex items-center justify-center">
                                <TrendingUp className="w-4 h-4 text-rust-600" />
                            </div>
                            <div>
                                <p className="text-xs font-bold text-surface-900">Efficiency Boost</p>
                                <p className="text-[10px] text-surface-500">+40% Productivity</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="p-8 sm:p-12 flex flex-col justify-center bg-white/40 overflow-y-auto">
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-surface-900 mb-2">Partner with Us</h1>
                    <p className="text-surface-500">Create your organizer profile</p>
                </div>
                ...
                <form className="space-y-5" onSubmit={handleSubmit}>
                    ...
                    <div className="flex items-center gap-4 mb-6">
                        <div className="relative group cursor-pointer shrink-0" onClick={() => fileInputRef.current?.click()}>
                            <div className="w-16 h-16 rounded-xl bg-white border-2 border-surface-200 flex items-center justify-center overflow-hidden transition-all duration-300 group-hover:border-rust-400 shadow-sm">
                                {profileImage ? (
                        ...
                        <div>
                            <p className="text-sm font-medium text-surface-900">Brand / Logo</p>
                    ...
                    <div className="text-center pt-2">
                        <p className="text-surface-500 text-sm">
                            Looking to book? <Link to="/signup/customer" className="text-rust-600 font-semibold hover:underline">Sign up as Customer</Link>
                        </p>
